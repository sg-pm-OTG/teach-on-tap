...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fetch emails for each admin user from auth.users
    const adminsWithEmails = await Promise.all(
      (adminRoles || []).map(async (adminRole) => {
        const { data: { user: adminUser } } = await supabaseAdmin.auth.admin.getUserById(adminRole.user_id);
        return {
          ...adminRole,
    ...
    console.log(`Fetched ${adminsWithEmails.length} admin users`);
